...
private val bmMapPlaylistTriggerConsumerSlots = Semaphore(NETWORK_HANDLER_CONCURRENCY)
private val bmPlaylistStatsConsumerSlots = Semaphore(NETWORK_HANDLER_CONCURRENCY)

fun Application.playlistStats() {
    rabbitOptional {
        consumeAck("bm.mapPlaylistTrigger", Int.serializer()) { _, mapId ->
            bmMapPlaylistTriggerConsumerSlots.withPermit {
                transaction {
        ...
        consumeAck("bm.playlistStats", Int.serializer()) { _, playlistId ->
            bmPlaylistStatsConsumerSlots.withPermit {
                transaction {
                    modelPostgresOperation()
